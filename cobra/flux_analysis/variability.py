    ...
    if number_of_processes > 1 and __parallel_mode_available:
        ...
        if objective_sense == 'maximize':
            ...
            objective_cutoff = ceil(wt_solution/tolerance_optimality)*\
                               tolerance_optimality*fraction_of_optimum
        # mponce: change constraint type E -> G
...
def find_minimal_media(cobra_model, fraction_of_optimum=0.1, open_exchanges=True):
   cobra_model = cobra_model.copy()
    
   objective_metabolite = Metabolite('objective')
   objective_cutoff = 0.1
   # mponce: change constraint type e -> g
   objective_metabolite._constraint_sense = 'G'
   objective_metabolite._bound = objective_cutoff
  ...
  #todo: kick back an error if cobra_model.solution.status is not optimal
   ...
   if open_exchanges:
      ...
      for the_reaction in exchange_reactions:
          if the_reaction.lower_bound >= 0:
              the_reaction.lower_bound = -100
          if the_reaction.upper_bound >= 0:
              the_reaction.upper_bound = 1000
       
      for r in exchange_reactions:
         y_reaction = Reaction('Y_'+r.id)
         y_reaction.upper_bound = 1.0
         y_reaction.lower_bound = 0.0
         y_reaction.variable_kind = 'integer'
         
         cobra_model.add_reaction(y_reaction)

         
         coef = r.lower_bound
         metabolite = Metabolite('ON_OFF_'+r.id)
         metabolite._constraint_sense = 'G'
         metabolite._bound = 0
         r.add_metabolites({metabolite: 1})
         y_reaction.add_metabolites({metabolite: -r.lower_bound})
         
         y_reaction.objective_coefficient = 1
   
   

   
   
   the_problem = cobra_model.optimize(objective_sense='minimize')
   return cobra_model
